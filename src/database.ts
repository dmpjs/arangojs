...
import { AqlLiteral, AqlQuery, isAqlLiteral, isAqlQuery } from "./aql.ts";
import {
  ArangoCollection,
  Collection,
  CollectionMetadata,
...
export class Database {
  ...
  listDatabases(): Promise<string[]> {
    return this.request({ path: "/_api/database" }, (res) => res.data.result);
  }
  ...
  databases(): Promise<Database[]> {
    return this.request(
      { path: "/_api/database" },
      (res) =>
        (res.data.result as string[]).map((databaseName) =>
          this.database(databaseName)
        ),
    );
  }
  ...
  listGraphs(): Promise<GraphInfo[]> {
    return this.request({ path: "/_api/gharial" }, (res) => res.data.graphs);
  }
  ...
  listViews(): Promise<ViewDescription[]> {
    return this.request({ path: "/_api/view" }, (res) => res.data.result);
  }
  ...
  listAnalyzers(): Promise<AnalyzerDescription[]> {
    return this.request({ path: "/_api/analyzer" }, (res) => res.data.result);
  }
  ...
  beginTransaction(
    ...
    return this.request(
      ...
      (res) => new Transaction(this, res.data.result.id),
    );
  }
   ...
   * @example
   * ```js
   * // Using a deno Uint8Array as source
   * const source = Deno.readFileSync("./my-foxx-service.zip");
   * const info = await db.installService("/hello", source);
   * ```
   */
  async installService(
    mount: string,
    source: ReadableStream | ArrayBuffer | string,
   ...
   * @example
   * ```js
   * // Using a node.js ArrayBuffer as source
  ...
  async upgradeService(
    mount: string,
    source: Deno.Reader | ArrayBuffer | string,
   ...
   * @param mount - The service's mount point, relative to the database.
   */
  downloadService(mount: string): Promise<ArrayBuffer> {
    return this.request(
      {
        method: "POST",
